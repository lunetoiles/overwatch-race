...
Rule type: Ongoing - Global
    ...
    Cond: Q == false //donft run during debug to keep inspector clean
...
**Global state 11 - get in game and prep unlocking**
    ...
    Create hud text gUNLOCKING CHECKPOINT AND PAYLOADh
    G <= last text id
    Create hud text gFIND TEAM 2 PLAYERSh text visible when no one on team 2
    H <= last text id
...
**Global state 19 - complete game set up**
    ...
    Set objective description (ggo fast!h)
...
**Global state 21 - Leaderboard set up part 2**
    ...
    Create in-world text(
    	...
    	Text: g{L[{index}]} - {L{index}]:W} Sech
    	Position: E
    	Scale: H
    	Clipping: Yes
    	Reevaluation: Visible to and string
...
**Player state 4 - hud creation part 4 - Latest times**
    ...
    Create g-------h text - sort 8
    Create gtimesh text - sort 9
    (repeat x5)
...
**Player state 20 and left spawn - Return player to gather point**
    ...
    Cond: gQ == false // donft force it in debug
...
**Player state 51- Update records**
    ...
    Skip if ( W >= gW ) { //skip if not new server record
    	gW <= W
    	Create big message (gNew High Score: {event player} - {W} Sech)
    	State <= 60
    	Abort
    }
    Create small message (g{event player} new record - {W} Sech)
    State <= 60
